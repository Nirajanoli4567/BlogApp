    ...
    <form action="{{ route('blog.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select id="category" class="form-select @error('category') is-invalid @enderror" name="category" required>
                ...
                <option value="art" {{ old('category', $blog->category) == 'art' ? 'selected' : '' }}>Art</option>
                <option value="education" {{ old('category', $blog->category) == 'education' ? 'selected' : '' }}>Education</option>
                <option value="it" {{ old('category', $blog->category) == 'it' ? 'selected' : '' }}>IT</option>
            </select>
            @error('category')
                <div class="invalid-feedback">
                    {{ $message }}
        ...
        <div class="mb-3">
            ...
            @if($blog->photo)
                <img src="{{ asset('storage/' . $blog->photo) }}" alt="Blog Photo" style="max-height: 100px; margin-top: 10px;">
            @endif
            @error('photo')
                <div class="invalid-feedback">
                    {{ $message }}
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4" required>{{ old('description', $blog->description) }}</textarea>
            @error('description')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
